...
def get_alert_config(config_dict: dict) -> None:
    ...
    if config_dict["tg_config"]:
        while True:
            user_input = console.input(f"Если цена товара выше или равна данного значения, уведомлять в TG \[{accent('Не уведомлять')}]:")
            if user_input == "":
                config_dict["price_min_value_alert"] = None
                break
            try:
                config_dict["price_min_value_alert"] = float(user_input)
                break
            except ValueError:
...
def create_config():
    ...
    if run:
        ...
        parser_instance = Parser_url(
            ...
            categories_path=config_dict["categories_path"],
            ...
            price_min_value_alert=config_dict["price_min_value_alert"],
