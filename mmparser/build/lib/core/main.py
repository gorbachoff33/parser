...
def run_url_parser(args: argparse.Namespace, config: dict = {}) -> None:
    parser_instance = Parser_url(
        categories_path=config.get("categories_path") or args.categories_path,
        url=config.get("url") or args.url,
        urls=config.get("urls", []) or (args.urls if args.urls else []),
        ...
        tg_config=config.get("tg_config", []) or (args.tg_config if args.tg_config else []),
        price_min_value_alert=config.get("price_min_value_alert") or args.price_min_value_alert,
...
def main():
    ...
    parser.add_argument("urls", nargs="*", help="Список URL-адресов")
    parser.add_argument("-categories-path", type=str, help="Категории товаров")
